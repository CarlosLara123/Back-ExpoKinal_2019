...
function agregarTexto(req, res) {
    ...
    if (params.titulo && params.descripcion) {
        ...
        texto.save((err, textoGuardado) => {
            ...
            if (textoGuardado) {
               return res.status(200).send({ texto: textoGuardado });
            } else {
                return res.status(404).send({ message: 'No a podido guardarse el Texto' });
            }
        })
    } else {
        res.status(200).send({
...
function subirImagen(req, res) {
    ...
    if (req.files) {
        ...
        if (file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif') {
            Texto.findByIdAndUpdate(textoId, { imagen: file_name }, { new: true }, (err, usuarioActualizado) => {
...
function getImageFile(req, res) {
    ...
    var path_file = './src/uploads/entradas/' + image_file;
...
function eliminarTexto(req, res) {
    ...
    Texto.findByIdAndDelete(textoId, (err, textoEliminado) => {
        ...
        if (!textoEliminado) 
            ...
            return res.status(200).send({ message: 'El texto Fue eliminado' });
...
function listarTexto(req, res) {
    ...
    Texto.find((err, TextoEncontrado) => {
        ...
        return res.status(200).send({ textos: TextoEncontrado });
    })
}
...
function buscarTexto(req,res){
    ...
    Texto.findOne({titulo: {$regex: new RegExp(NombreT)}},(err,TextoEnc)=>{
