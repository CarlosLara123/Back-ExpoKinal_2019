...
function buscarHistoria(req, res) {
    var historiaId = req.params.id

    Historia.findById(historiaId,(err, historias)=>{
        if(err) return res.status(500).send({message: 'error en la peticion'});

        if(!historias) return res.status(404).send({message: 'no se a podido encontrar la parte de la historia'});
...
function subirImagen(req, res) {
    ...
    if(req.files){
        ...
        if(file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif'){
            Historia.findByIdAndUpdate(userId, {imagen: file_name}, {new:true},(err, histAct)=>{
                if(err) return res.status(500).send({message: 'Error en la peticion'})
...
function getImageFile(req, res) {
    ...
    var path_file = './src/uploads/historia/' + image_file;
