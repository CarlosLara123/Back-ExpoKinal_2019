...
function registrarcarrousel(req, res) {
...
function subirImagen(req, res) {
    ...
    if(req.files){
        ...
        if(file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif'){
            Carrousel.findByIdAndUpdate(userId, {image: file_name}, {new:true},(err, usuarioActualizado)=>{
                if(err) return res.status(500).send({message: 'Error en la peticion'})
...
function getImageFile(req, res) {
    ...
    var path_file = './src/uploads/carrousel/' + image_file;
